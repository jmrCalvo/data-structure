...
Nmer prof;
cout<<"el tamaño es "<<prof.size()<<endl;
prof.loadSerialized("../datos/cadenaSimple.srl");
//prof.list_Nmer();
cout << "el tamaño es "<<prof.size() << " " << prof.length() << endl;

// -------------- PRUEBA --------------
string adn = "ACCATACTCCAAG";
cout<<"***************sequenceADN**********************"<<endl;
prof.sequenceADN(4,adn);
cout <<"la altura antes es : "<< prof.length() << endl;
cout<<"***************Prefix**********************"<<endl;
Nmer pruebas=prof.Prefix("A");
cout << "la Altura " << pruebas.length() << endl;
cout<<"***************list_Nmer**********************"<<endl;
pruebas.list_Nmer();
cout<<"***************containsString**********************"<<endl;
bool result=prof.containsString("CATA");
if(result){cout<<"CATA está"<<endl;}
else{cout<<"CATA no está"<<endl;}
cout<<"***************Union**********************"<<endl;
string primer="CACAGTAGTCAC",segund="CACA";
Nmer primero,segundo;
primero.sequenceADN(4,primer);
segundo.sequenceADN(4,segund);
Nmer solucion=primero.Union(segundo);
solucion.list_Nmer();
cout<<"***************included**********************"<<endl;
if(primero.included(segundo))cout<<"lo contiene"<<endl;
else cout<<"no lo contiene"<<endl;
cout<<"***************Distance**********************"<<endl;
cout<<"la distancia entre uno y si mismo debe ser 0 y el resultado es "<<primero.Distance(primero)<<endl;
cout<<"***************rareNmer**********************"<<endl;
  //for(auto it=primero.rareNmer(3).begin();it!=primero.rareNmer(3).end();++it){
  //cout<<" la cadena es "<< it->first<<" y su frecuencia es "<<it->second<<endl;
//}
cout<<"***************commonNmer**********************"<<endl;
Nmer otro;string lo="ACACACGTGTAGACACA";
//for(auto it=otro.commonNmer(3).begin();it!=otro.commonNmer(3).end();++it){
  //cout<<" la cadena es "<< it->first<<" y su frecuencia es "<<it->second<<endl;
// }
cout<<"***************level**********************"<<endl;

otro.sequenceADN(4,lo);
for(auto it=otro.level(3).begin();it!=otro.level(3).end();++it){
  cout<<" la cadena es "<< it->first<<" y su frecuencia es "<<it->second<<endl;
}
return 0;
}
